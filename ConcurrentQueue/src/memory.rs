...
impl MemoryManagerInner {
    ...
    pub fn getToken(&mut self, at: usize) -> *const MemToken {
    ...
    pub fn removeToken(&mut self, token: *const MemToken) {
        self.tokens.retain(|x| *x != token);
    }

    pub fn tryFreeing(&mut self, at: usize) -> bool {
    ...
    pub fn addFreeable(&mut self, vec: Vec<ToFree>) {
        self.tofree = vec;
    }
}
...
impl MemoryManager {
    ...
    pub fn get_token(&self) -> *const MemToken {
        ...
        inner.getToken(self.epoch.load(Ordering::Acquire))
    }

    pub fn remove_token(&self, token: *const MemToken) {
        self.update_token(token);
        let mut inner = self.mem_manager.lock().unwrap();
        inner.removeToken(token);
    ...
    fn start_free(&self, elemvec: &mut Vec<ToFree>) {
        let _ = self.mem_manager.try_lock().map(|mut inner| {
            ...
            if inner.epoch == cur_epoch {
                ...
                inner.addFreeable(newv);
                self.epoch.store(cur_epoch.wrapping_add(1), Ordering::Release);
                self.signal.setEpoch(Ordering::Release);
            }
        });
    }
    ...
    pub fn free<T>(&self, pt: *mut T, num: usize) {
        ...
        elemvec.push(ToFree::new(pt, num));
        {
            let _ = self.mem_manager.try_lock().map(|mut inner| {
                ...
                if inner.tryFreeing(epoch) {
                    self.signal.clearEpoch(Ordering::Release);
                }
            });
        }
        if elemvec.len() > 20 {
