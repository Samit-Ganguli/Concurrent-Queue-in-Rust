...
impl<RW: QueueRW<T>, T> InnerRecv<RW, T> {
    ...
    pub fn add_stream(&self) -> InnerRecv<RW, T> {
        InnerRecv {
            ...
            reader: self.queue.tail.addStream(&self.reader, &self.queue.manager),
    ...
    unsafe fn do_unsubscribe_with<F: FnOnce()>(&mut self, f: F) {
        if self.alive {
            self.alive = false;
            if self.reader.removeConsumer() == 1 {
                if self.queue.tail.removeReader(&self.reader, &self.queue.manager) {
                    self.queue
                        .manager
                        .signal
                        .setReader(SeqCst);
